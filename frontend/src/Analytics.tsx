...
export default function Analytics({ user }: { user: User }) {
  ...
  return <section className="ca-analytics" aria-label="Hospital analytics"><div className="ca-heading"><div><h2>Hospital analytics</h2><p>{days[0]} to {end} · Bed and stock figures show current state.</p></div><div className="ca-heading-actions"><label>Reporting period<select value={period} onChange={e => setPeriod(Number(e.target.value))}><option value={7}>Last 7 days</option><option value={30}>Last 30 days</option><option value={90}>Last 90 days</option></select></label><button className="ca-details-toggle" type="button" aria-expanded={showDetails} onClick={() => setShowDetails(value => !value)}><span>{showDetails ? 'Hide details' : 'Show details'}</span><ChevronDown size={17} /></button></div></div><div className="ca-grid ca-primary-grid">
  ...
  </div>{showDetails && <div className="ca-grid ca-secondary-grid">
