...
export default function Overview({ user, navigate, onAddPatient, onSchedule, ready }: { user: User; navigate: (page: string) => void; onAddPatient: () => void; onSchedule: () => void; ready: boolean }) {
  ...
  const staffAllowed = user.role === 'Admin';
  const patients = usePatients(); const appointments = useAppointments(); const staff = useStaff(); const beds = useBeds(bedsAllowed);
  ...
  const metrics = [
    ...
    { title: 'Clinical team', value: staff.data?.length, icon: Stethoscope, color: 'purple', caption: staffAllowed ? 'Staff records' : 'Restricted for your role', page: 'staff', error: staff.isError, restricted: !staffAllowed },
    { title: 'Available beds', value: beds.data?.available, icon: BedDouble, color: 'orange', caption: bedsAllowed ? 'Ready for admission' : 'Restricted for your role', page: 'beds', error: beds.isError, restricted: !bedsAllowed },
  ];
  return <><PageHeading eyebrow={date} title={`Good ${greetingForHour(currentHour)}, ${user.username.split(/[ _]/)[0]}.`} description="A clear picture of your day. More time for your patients."><button className="cf-button cf-button-primary" onClick={onAddPatient} disabled={!canManageCare} title={!canManageCare ? 'Your role can view patients but cannot add them' : undefined}><Plus size={17} /> Add patient</button></PageHeading>
    <div className="cf-metrics">{metrics.map(m => <button className="cf-metric" key={m.title} onClick={() => navigate(m.page)} disabled={m.restricted} aria-label={m.restricted ? `${m.title}: restricted for your role` : undefined}><div className="cf-metric-top"><span className={`cf-icon-tile cf-${m.color}`}><m.icon size={20} /></span>{!m.restricted && <ArrowUpRight size={17} />}</div><span className="cf-metric-title">{m.title}</span><strong>{m.restricted ? <Badge status="Restricted access" /> : m.error ? 'Unable to load' : m.value ?? <span className="cf-skeleton" />}</strong><span className="cf-metric-caption">{m.restricted ? 'Your role does not have access to this data.' : m.error ? 'Open to retry' : m.caption}</span></button>)}</div>
    <div className="cf-overview-grid"><section className="cf-panel cf-schedule"><PanelHeading title="Today’s appointments" subtitle={`${todaysVisits.length} visits on the schedule`} action="View all" onAction={() => navigate('appointments')} />
      ...
      <section className="cf-panel cf-shortcuts"><PanelHeading title="Make your next move" subtitle="Your everyday essentials, close by" />{[{ title: 'Register a patient', sub: 'Start a new care journey', icon: Users, color: 'green', run: onAddPatient, disabled: !canManageCare }, { title: 'Schedule an appointment', sub: 'Find a time for the next visit', icon: CalendarDays, color: 'blue', run: onSchedule, disabled: !ready || !canManageCare }, { title: 'Manage beds', sub: bedsAllowed ? 'Coordinate admissions and availability' : 'Restricted for your role', icon: BedDouble, color: 'orange', run: () => navigate('beds'), disabled: !bedsAllowed }].map(s => <button className="cf-shortcut" key={s.title} onClick={s.run} disabled={s.disabled}><span className={`cf-icon-tile cf-${s.color}`}><s.icon size={19} /></span><span><strong>{s.title}</strong><small>{s.sub}</small></span>{!s.disabled && <ArrowUpRight size={17} />}</button>)}</section>
